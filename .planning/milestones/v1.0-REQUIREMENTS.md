...
- [x] **CONN-01**: App parses ~/.ssh/config to display available servers
- [x] **CONN-02**: User can add and save custom SSH connections
- [x] **CONN-03**: User can connect to server using SSH key authentication
...
- [x] **FILE-05**: User can move files to different folder on server *(backend ready, UI deferred - needs remote folder picker)*
...
| FILE-05 | Phase 5 | ⚠️ Partial (backend ready, UI deferred) |
...
**Shipped:** 17 of 18 v1 requirements (94.4%)

**Adjusted during implementation:**
- NAV-05 (smart column scrolling) was added during post-v1 polish based on user testing
- FILE-05 (move files) was scoped down to backend-only after discovering native dialogs can't browse remote folders
